...
export function suggestAcademicYearDates(yearLabel: string): { startDate: string; endDate: string } {
...
export function suggestTermsForYear(
  ...
  if (Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || count < 1) {
    return []
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    const termEnd =
      i === count - 1
        ? new Date(end)
        : new Date(start.getTime() + daysPerTerm * (i + 1) * 24 * 60 * 60 * 1000 - 1)

    terms.push({
      name: count === 2 ? (i === 0 ? 'Semester 1' : 'Semester 2') : `Term ${i + 1}`,
...
export const STREAM_NAME_SUGGESTIONS = ['A', 'B', 'East', 'West', 'North', 'South', 'Main']

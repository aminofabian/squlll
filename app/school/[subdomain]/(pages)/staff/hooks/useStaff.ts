  ...
  const fetchStaff = async () => {
    try {
      ...
      try {
        ...
        tenantId =
          configStore.useSchoolConfigStore.getState().config?.tenant?.id;
      } catch {
        // Tenant ID not available — proceed without it
      }
      ...
      if (err.response?.errors) {
        ...
        if (
          ...
          setError(
            `The GraphQL API doesn't have the 'getAllStaff' query available. Error: ${graphQLErrors[0].message}`,
          );
        } else if (
          graphQLErrors.some(
    ...
    try {
      ...
      if (response.ok && result.data?.getAllStaff) {
        const transformedStaff =
          result.data.getAllStaff.map(transformStaffData);
        console.log(
          `Direct fetch succeeded! Retrieved ${transformedStaff.length} staff members`,
        );
        setStaff(transformedStaff);
        setError(null);
        setLoading(false);

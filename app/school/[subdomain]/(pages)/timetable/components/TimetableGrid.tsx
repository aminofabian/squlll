...
interface TimetableGridProps {
  ...
  onCellClick: (timeId: string, dayIndex: number) => void;
  onTimeSlotClick: (timeId: string) => void;
  onInputChange: (value: string) => void;
  onTimeSlotEditChange: (value: string) => void;
  onTeacherChange: (teacher: string) => void;
  onInputSubmit: () => void;
  onTimeSlotSave: (timeId: string) => void;
  onCancelEdit: () => void;
  ...
  getCellKey: (grade: string, timeId: string, dayIndex: number) => string;
  onResetTimeSlot?: (timeId: string) => void;
}
...
export const TimetableGrid: React.FC<TimetableGridProps> = ({
  ...
  // Group slots by day and compute period list
  ...
  const getSlotFor = (dayIndex: number, period: number): TimeSlot | undefined => {
    const daySlots = slotsByDay[dayIndex + 1] || [];
    return daySlots.find((s) => s.periodNumber === period);
  };

  const getBaseSlotForPeriod = (period: number): TimeSlot | undefined => {
  ...
  const getDayStats = (dayIndex: number) => {
    const daySlots = slotsByDay[dayIndex + 1] || [];
    const totalSlots = daySlots.length || periodNumbers.length;
    const filledSlots = daySlots.reduce((count, slot) => {
      const cellKey = getCellKey(selectedGrade, slot.id, dayIndex);
      return subjects[cellKey] ? count + 1 : count;
    }, 0);
    const hasConflicts = daySlots.some((slot) => conflicts[getCellKey(selectedGrade, slot.id, dayIndex)]);
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50 px-4 py-3">
          <div className="space-y-3">
            {periodNumbers.map((period, index) => {
              const slot = getSlotFor(mobileViewDay, period);
              const displaySlot = slot || getBaseSlotForPeriod(period);
              if (!displaySlot) return null;
              const cellKey = slot ? getCellKey(selectedGrade, slot.id, mobileViewDay) : `missing-${period}-${mobileViewDay}`;
              ...
              return (
                <div key={`${period}-${mobileViewDay}`} className="relative">
                  {/* Timeline Item */}
                  <div className="flex items-start gap-4">
                    {/* Time Badge */}
                    <div 
                      className={`flex-shrink-0 ${slot ? 'cursor-pointer' : 'cursor-default'}`}
            ...
            return (
              <div 
                ...
                className={`grid grid-cols-6 gap-0 border-b border-gray-200 last:border-b-0 transition-all duration-300 ${
                  index === periodNumbers.length - 1 && showTimeSlotSuccess ? 'bg-green-50/30 shadow-lg' : ''
                }`} 
              ...
              >
                    ...
                    <>
                      ...
                      <div className="absolute right-2 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            if (onResetTimeSlot) onResetTimeSlot(baseSlot.id);
                          }}
                          className={`p-1 ${onResetTimeSlot ? 'text-primary hover:text-primary/80' : 'text-slate-400 cursor-not-allowed'}`}
                          title="Reset periods"
                ...
                {days.map((day, dayIndex) => {
                  const slot = getSlotFor(dayIndex, period);
                  const cellKey = slot ? getCellKey(selectedGrade, slot.id, dayIndex) : `missing-${period}-${dayIndex}`;
                  const isEditing = editingCell === cellKey;
                  const cellData = subjects[cellKey];
                  const hasConflict = conflicts[cellKey];
                  const teacher = cellData?.teacher ? teachers[cellData.teacher] : null;

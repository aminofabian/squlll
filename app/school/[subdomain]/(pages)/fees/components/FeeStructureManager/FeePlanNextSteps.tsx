...
export function FeePlanNextSteps({
  ...
  return (
    <div
      className={cn(
        FEES_MOBILE.card,
        "border-amber-200/80 bg-amber-50/95 px-4 py-3.5 max-md:border md:rounded-none md:border md:px-3 md:py-2.5",
        className,
      )}
      role="status"
    >
      <div className="flex items-start gap-2.5">
        <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-none border border-amber-200/80 bg-amber-100 text-amber-800">
        ...
        <div className="min-w-0 flex-1">
          ...
          <ul className="mt-2 space-y-1.5">
            {items.map((item) => (
              ...
              >
                <span
                  className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-none bg-amber-500"
                  aria-hidden
                />
                <span>{item}</span>
              </li>

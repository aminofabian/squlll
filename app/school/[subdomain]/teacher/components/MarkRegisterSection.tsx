...
import { CheckCircle2, XCircle, ChevronLeft, Loader2, CalendarDays, Users, ClipboardList, Home } from "lucide-react";
...
const MARK_ATTENDANCE_MUTATION = `
  mutation MarkAttendance($markAttendanceInput: CreateAttendanceInput!) {
...
function studentDisplayName(student: Student): string {
  return student.user?.name?.trim() || student.admission_number;
}
...
export default function MarkRegisterSection({
  ...
  const gradeLevels = useMemo(() => {
    if (teacher) return getFormattedGradeLevels();
    return schoolGradeLevels.flatMap((level) =>
      (level.grades || []).map((grade) => ({
        id: grade.id,
        name: grade.name,
      })),
    );
  }, [teacher, getFormattedGradeLevels, schoolGradeLevels]);
  ...
  useEffect(() => {
    if (gradeLevels.length > 0 && !selectedGradeLevelId) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedGradeLevelId || students.length === 0) return;
    ...
    try {
      await graphqlClient.request(MARK_ATTENDANCE_MUTATION, {
        markAttendanceInput: input,
      });
      setSuccess(true);
    } catch (err) {
  ...
  if (teacherLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-16 animate-fadeIn min-h-screen">
        ...
        <div className="text-lg font-semibold text-primary mb-2">Loading...</div>
        <div className="text-muted-foreground text-sm">Fetching your assigned classes.</div>
      </div>
    );
  }
  ...
  if (teacherError && !teacher) {
    return (
      <div className="flex flex-col items-center justify-center py-16 animate-fadeIn min-h-screen">
        ...
        <div className="text-lg font-semibold text-destructive mb-2">Unable to Load Profile</div>
        <div className="text-muted-foreground text-sm mb-6">{teacherError}</div>
        <button
  ...
  if (gradeLevels.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 animate-fadeIn min-h-screen">
        <div className="w-16 h-16 flex items-center justify-center bg-muted text-muted-foreground mb-4 shadow-lg">
          <Users className="w-8 h-8" />
        </div>
        <div className="text-lg font-semibold text-foreground mb-2">No Classes Assigned</div>
        <div className="text-muted-foreground text-sm mb-6 text-center max-w-md">
          You don&apos;t have any grade levels assigned yet. Contact your administrator to mark attendance.
        </div>
        <button
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 animate-fadeIn min-h-screen">
        ...
        <div className="text-muted-foreground text-sm mb-6">
          Attendance for {selectedGrade?.name ?? "the class"} has been saved. Students and parents will be notified.
        </div>
        <button
          className="px-4 py-2 bg-primary text-white font-semibold hover:bg-primary/90 transition shadow"
          onClick={onBack}
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-primary/5 to-white py-4 px-1 animate-fadeIn overflow-x-hidden">
      <form
        onSubmit={handleSave}
        className="w-full max-w-2xl mx-auto bg-white shadow-2xl border border-primary/10 p-0 flex flex-col gap-6 sm:gap-10 overflow-hidden min-h-[80vh]"
      >
        <div className="relative bg-gradient-to-br from-primary/10 via-primary/5 to-white shadow-sm px-2 sm:px-8 pt-8 pb-6 flex flex-col items-center gap-4 border-b border-primary/10">
          ...
          <div className="w-full max-w-xs sm:max-w-sm">
            <label className="block text-sm font-medium text-muted-foreground mb-1.5 text-center">
              Select class
            </label>
            <Select
          ...
          {selectedGrade && (
            <span className="inline-flex items-center gap-2 px-4 py-1.5 bg-primary/70 text-white font-semibold shadow">
              <Users className="w-5 h-5" /> {selectedGrade.name}
              {!studentsLoading && students.length > 0 && (
                <span className="opacity-90">· {students.length} students</span>
              )}
            </span>
          )}
        ...
        {error && (
          <div className="mx-4 sm:mx-8 px-4 py-3 bg-destructive/10 text-destructive text-sm rounded border border-destructive/20">
            {error}
          </div>
        )}
        ...
        {studentsLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 px-4">
            ...
            <p className="text-muted-foreground text-sm text-center">
              No students enrolled in {selectedGrade?.name ?? "this class"} yet.
            </p>
          </div>
        ) : (
          <>
              ...
              <table className="min-w-full border overflow-hidden shadow-sm text-xs sm:text-sm">
                <thead>
                  <tr className="bg-primary/10 text-primary text-left">
                    ...
                    <th className="px-2 sm:px-4 py-2 sm:py-3 font-semibold">Student</th>
                ...
                <tbody>
                  {studentsToShow.map((student, idx) => (
                    ...
                    >
                      <td className="px-2 sm:px-4 py-2 sm:py-3 text-muted-foreground font-mono">
                        {idx + 1}
                      </td>

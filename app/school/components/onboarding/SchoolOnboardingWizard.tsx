...
import { BookOpen, GraduationCap, Loader2, Rocket, UserPlus } from 'lucide-react'
...
export function SchoolOnboardingWizard() {
  ...
  const { academicYears, loading: yearsLoading, refetch: refetchYears } = useAcademicYears()
  const { data: schoolConfig, refetch: refetchConfig } = useSchoolConfig()

  const [academicYearForm, setAcademicYearForm] = useState(buildInitialYearForm)
  const [createdAcademicYear, setCreatedAcademicYear] = useState<AcademicYear | null>(null)
  const [isCreatingYear, setIsCreatingYear] = useState(false)

  const [termMode, setTermMode] = useState<'suggested' | 'moe' | 'custom'>('suggested')
  const [suggestedTermCount, setSuggestedTermCount] = useState(3)
  const [termDrafts, setTermDrafts] = useState<TermDraft[]>([])
  const [customTerm, setCustomTerm] = useState<TermDraft>({ name: '', startDate: '', endDate: '' })
  ...
  const termsForYear = useMemo(() => {
    ...
    return academicYears.find((y) => y.id === activeAcademicYear.id)?.terms || []
  ...
  }, [yearsLoading, schoolConfig, hasAcademicYear, hasTerms, gradesWithoutStreams.length, router])

  const setYearField = (field: 'name' | 'startDate' | 'endDate', value: string) => {
  ...
  const applyMoeYear = () => {
    ...
    setAcademicYearForm({ name: moe.name, startDate: moe.startDate, endDate: moe.endDate })
    toast.message(`Loaded ${moe.year} MoE calendar — you can still edit the dates`)
  }

  const handleCreateAcademicYear = async () => {
    if (!academicYearForm.name.trim() || !academicYearForm.startDate || !academicYearForm.endDate) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || 'Failed to create academic year')
  ...
  const applySuggestedTerms = () => {
    ...
    const end = activeAcademicYear.endDate.split('T')[0]
    setTermDrafts(suggestTermsForYear(start, end, suggestedTermCount))
    toast.message(`Generated ${suggestedTermCount} terms — edit names or dates below`)
  }
  ...
  const handleAddCustomTerm = () => {
    if (!customTerm.name.trim() || !customTerm.startDate || !customTerm.endDate) {
      toast.error('Enter term name, start date, and end date')
      return
    }
    setTermDrafts((prev) => [...prev, { ...customTerm, name: customTerm.name.trim() }])
  ...
  const handleCreateTermDrafts = async () => {
    ...
    for (const draft of termDrafts) {
      try {
        ...
        const result = await response.json()
        if (!response.ok) throw new Error(result.error || `Failed to create ${draft.name}`)
        created++
      } catch (err) {
  ...
  const handleCreateStreams = async () => {
    const jobs: { gradeId: string; gradeName: string; name: string; capacity: string }[] = []
    ...
    for (const grade of gradeRows) {
      ...
      for (const draft of plans) {
        ...
        if (!capacity || Number(capacity) < 1) {
          toast.error(`Capacity for "${name}" on ${grade.gradeName} must be at least 1`)
          return
        }

        jobs.push({ gradeId: grade.gradeId, gradeName: grade.gradeName, name, capacity })
      }
    }
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 2:
        return (
          <OnboardingStep icon={BookOpen} title="Terms" description="Split the year into teaching periods.">
            <TermsStepContent
              hasAcademicYear={hasAcademicYear}
              hasTerms={hasTerms}
        ...
        return (
          ...
          >
            <ul className="space-y-3">
              <ChecklistItem title="Teachers" description="Staff who teach or manage classes." href="/teachers" />
              <ChecklistItem title="Students" description="Enroll learners and assign streams." href="/students" />
              <ChecklistItem title="Fees" description="Fee structures for the active term." href="/fees" />
              <ChecklistItem title="Timetable" description="Periods, breaks, and lessons." href="/timetable" />
            </ul>
          </OnboardingStep>
        )
      ...
      case 5:
        return (
          <OnboardingStep icon={Rocket} title="All set" description="Your school is ready for day-to-day use.">
            <div className="space-y-2 mb-4">
              ...
              <SummaryRow
                ...
                label="Class streams"
                hint={hasAnyStream ? undefined : 'You can add these from Classes'}
              />
            ...
            <Card className="bg-primary/5 border-primary/20">
              <CardContent className="pt-4">
                <p className="text-sm text-muted-foreground">
                  Your dashboard is where you manage daily operations. You can return to any skipped step
                  from the sidebar.
                </p>
              </CardContent>
            </Card>
...
function SummaryRow({
  ...
  return (
    <div className="flex items-start gap-2 text-sm">
      <span className={`mt-0.5 h-2 w-2 rounded-full shrink-0 ${done ? 'bg-emerald-500' : 'bg-slate-300'}`} />
      <span>
